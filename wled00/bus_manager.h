...
struct BusConfig
{
  ...
  bool adjustBounds(uint16_t &total)
  {
    if (!count)
      count = 1;
    if (count > MAX_LEDS_PER_BUS)
      count = MAX_LEDS_PER_BUS;
    if (start >= MAX_LEDS)
      return false;
...
public:
  Bus(uint8_t type, uint16_t start, uint8_t aw)
      : _bri(255), _len(1), _valid(false), _needsRefresh(false)
  {
    _type = type;
    _start = start;
  ...
  virtual bool hasRGB()
  {
    if ((_type >= TYPE_WS2812_1CH && _type <= TYPE_WS2812_WWA) || _type == TYPE_ANALOG_1CH || _type == TYPE_ANALOG_2CH || _type == TYPE_ONOFF)
      return false;
    return true;
  }
  ...
  virtual bool hasCCT()
  {
    if (_type == TYPE_WS2812_2CH_X3 || _type == TYPE_WS2812_WWA ||
        _type == TYPE_ANALOG_2CH || _type == TYPE_ANALOG_5CH)
      return true;
    return false;
  }
...
// compile-time limiter for hardware that can't power both white channels at max
    ...
    if (_cctBlend > WLED_MAX_CCT_BLEND)
      _cctBlend = WLED_MAX_CCT_BLEND;
#endif
  }
  inline void setAutoWhiteMode(uint8_t m)
  {
    if (m < 5)
      _autoWhiteMode = m;
  }
  inline uint8_t getAutoWhiteMode() { return _autoWhiteMode; }
  inline static void setGlobalAWMode(uint8_t m)
  {
    if (m < 5)
      _gAWM = m;
    else
      _gAWM = AW_GLOBAL_DISABLED;
  }
...
public:
  ...
  {
    if (busId >= WLED_MAX_BUSSES || busId > numBusses || busses[busId] == nullptr)
      return -1;
